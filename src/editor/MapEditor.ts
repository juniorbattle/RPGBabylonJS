 ...
 * - Sol : DynamicTexture procédurale riche (palette par biome, taches organiques, vignette)
 * - Ciel : DynamicTexture procédurale (gradient + soleil + nuages + silhouettes)
 * - Props : Système de composition procédurale par groupes (ProceduralComposer)
 *           Groupes composites : anchor imposant + fills + accents
 *           3 couches de profondeur, grille toujours dégagée
 */
...
// ---------------------------------------------------------------------------
// ProceduralComposer — système de composition par groupes
// ---------------------------------------------------------------------------

/**
 * Un CompositeGroup décrit une composition visuelle intentionnelle :
 *  - anchor  : pièce maîtresse (grand arbre, ruine, falaise…)
 *  - fills   : props moyens placés en anneau autour de l'anchor
 *  - accents : petits props saupoudrés dans la zone du groupe
 */
...
const COMPOSITE_LIBRARY: CompositeGroup[] = [
    ...
    // ── Torche animée entourée de piliers ─────────────────────────────────
    {
        anchorType: "torch_flicker",
    ...
    // ── Tapis de fleurs (layer front/mid) ─────────────────────────────────
    {
        anchorType: "flower",
...
export class MapEditor {
    ...
    // ── Pools internes pour la composition procédurale ──────────────────────
    private _staticPool:   Map<string, PropDef[]>         = new Map();
    private _animatedPool: Map<string, AnimatedPropDef[]> = new Map();
    ...
    // -------------------------------------------------------------------------
    // Génération procédurale — système de groupes composites
    // -------------------------------------------------------------------------
    ...
    /**
     * Reconstruit les pools par type depuis le manifest + biome actif.
     * Appelé automatiquement au début de runProceduralGroundProps.
     */
    private buildCompositionPools(def: BiomeDef): void {
        this._staticPool.clear();
        this._animatedPool.clear();

        for (const [type, weight] of Object.entries(def.propWeights ?? {})) {
            if (weight <= 0) continue;
            const arr = this.allProps.filter(p => p.type === type);
            if (arr.length) this._staticPool.set(type, arr);
        }
        for (const [type, weight] of Object.entries(def.animatedPropWeights ?? {})) {
            if (weight <= 0) continue;
            const arr = this.allAnimatedProps.filter(p => p.type === type);
            if (arr.length) this._animatedPool.set(type, arr);
        }
    }

    /** Vérifie si un type est disponible (statique ou animé) dans les pools courants. */
    private hasPoolType(type: string): boolean {
        return this._staticPool.has(type) || this._animatedPool.has(type);
    }

    /** Pioche un def aléatoire depuis les pools (animé prioritaire si existant). */
    private pickFromPool(
        type: string,
        rng: () => number
    ): { def: PropDef | AnimatedPropDef; animated: boolean } | null {
        const anim = this._animatedPool.get(type);
        if (anim?.length) return { def: anim[Math.floor(rng() * anim.length)], animated: true };
        const stat = this._staticPool.get(type);
        if (stat?.length) return { def: stat[Math.floor(rng() * stat.length)], animated: false };
        return null;
    }

    /**
     * Choisit un groupe composite compatible avec le layer et le biome.
     * Le poids de sélection est proportionnel au poids de l'anchor dans le biome.
     */
    private selectCompositeGroup(
        layer: PropLayer,
        def: BiomeDef,
        rng: () => number
    ): CompositeGroup | null {
        const allWeights = { ...def.propWeights, ...(def.animatedPropWeights ?? {}) };
        const eligible = COMPOSITE_LIBRARY.filter(g =>
            g.allowedLayers.includes(layer) &&
            this.hasPoolType(g.anchorType) &&
        ...
        return picked !== null ? eligible[parseInt(picked)] : null;
    }

    /**
     * Spawn un prop unique (interne, utilisé par spawnCompositeGroup).
     * Gère automatiquement statique vs animé.
     */
    private spawnCompositeOne(
        type: string,
        ...
        const result = this.pickFromPool(type, rng);
        if (!result) return;

        const { def, animated } = result;
        const [sMin, sMax] = def.scaleRange;
        const finalScale = (sMin + rng() * (sMax - sMin)) * scaleMult;
        const mirror = rng() > 0.5;

        if (animated) {
            this.spawnAnimatedProp(def as AnimatedPropDef, px, py, pz, mirror, finalScale, isProcedural, layer);
        } else {
            this.spawnProp(def.file, px, py, pz, mirror, finalScale, isProcedural, layer);
        }
    }

    /**
     * Spawn un groupe composite complet (anchor + fills + accents).
     */
    private spawnCompositeGroup(
        ...
        // 1. Anchor
        const anchorScale = group.anchorScaleMult[0] + rng() * (group.anchorScaleMult[1] - group.anchorScaleMult[0]);
        this.spawnCompositeOne(group.anchorType, cx, cy, cz, anchorScale, layer, rng);

        // 2. Fills — anneau autour de l'anchor
        for (const fill of group.fills) {
            if (!this.hasPoolType(fill.type)) continue;
            const count = fill.count[0] + Math.floor(rng() * (fill.count[1] - fill.count[0] + 1));
            const spread = ((fill.angularSpread ?? 360) * Math.PI) / 180;
            const baseAngle = rng() * Math.PI * 2;
            for (let i = 0; i < count; i++) {
                const angle  = baseAngle + (i / count) * spread + (rng() - 0.5) * 0.8;
                const radius = fill.radiusRange[0] + rng() * (fill.radiusRange[1] - fill.radiusRange[0]);
                const scaleMult = fill.scaleMult[0] + rng() * (fill.scaleMult[1] - fill.scaleMult[0]);
                this.spawnCompositeOne(
                    fill.type,
        ...
        // 3. Accents — saupoudrage libre
        for (const accent of group.accents) {
            if (!this.hasPoolType(accent.type)) continue;
            const count = accent.count[0] + Math.floor(rng() * (accent.count[1] - accent.count[0] + 1));
            for (let i = 0; i < count; i++) {
                const angle  = rng() * Math.PI * 2;
                const radius = accent.radiusRange[0] + rng() * (accent.radiusRange[1] - accent.radiusRange[0]);
                const scaleMult = accent.scaleMult[0] + rng() * (accent.scaleMult[1] - accent.scaleMult[0]);
                this.spawnCompositeOne(
                    accent.type,
                    cx + Math.cos(angle) * radius,
                    cy,
    ...
    /**
     * Remplit une zone rectangulaire avec des groupes composites.
     * Utilise un rejet de proximité (Poisson-disc simplifié) pour éviter
     * la superposition des anchors.
     */
    private fillZoneWithGroups(
        x0: number, x1: number,
        z0: number, z1: number,
        layer: PropLayer,
        def: BiomeDef,
        ...
        // Nombre de groupes cible
        const count = Math.max(0, Math.floor((w * d / 14) * this.procDensity * densityMult));

        // Poisson-disc simplifié : distance minimale entre anchors
        const MIN_ANCHOR_DIST = 2.8;
        const placed: [number, number][] = [];

        for (let attempt = 0; attempt < count * 5 && placed.length < count; attempt++) {
            const cx = x0 + rng() * w;
            const cz = z0 + rng() * d;
            ...
            for (const [px, pz] of placed) {
                ...
                if (dx * dx + dz * dz < MIN_ANCHOR_DIST * MIN_ANCHOR_DIST) {
                    tooClose = true;
                    break;
                }
            }
            ...
            const group = this.selectCompositeGroup(layer, def, rng);
            if (!group) continue;

            placed.push([cx, cz]);
            this.spawnCompositeGroup(group, cx, this.currentGridElevation, cz, layer, rng);
        }
    }

    /**
     * Point d'entrée principal de la génération procédurale.
     * Remplace l'ancienne logique runProceduralGroundProps + spawnZone.
     */
    public runProceduralGroundProps(): void {
        // Effacer les props procéduraux existants
        ...
        // Construire les pools depuis le biome actif
        this.buildCompositionPools(def);

        const { xMin, xMax, zMin, zMax } = this.getGridBounds();
        const rng  = makeRng(this.procSeed);
        const SAFE = MapEditor.SAFE_CLEARANCE;
        const mBack  = this.tileSize * 5.5;
        const mSide  = this.tileSize * 5.0;
        const mFront = this.tileSize * 2.5;

        // ── BACK — groupes imposants (arbres majestueux, falaises, ruines) ──
        this.fillZoneWithGroups(
        ...
        // ── AVANT GAUCHE — végétation basse, herbes, fleurs ─────────────────
        this.fillZoneWithGroups(
